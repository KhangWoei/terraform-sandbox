#!/usr/bin/env bash
#wait for sqlserver to finish initializing - hacky
...
/opt/mssql-tools/bin/sqlcmd -H localhost -U sa -P $SA_PASSWORD -d master -i $SCRIPT_DIR/references/northwind/Northwind.sql

echo Installing Chinook...
/opt/mssql-tools/bin/sqlcmd -H localhost -U sa -P $SA_PASSWORD -d master -i $SCRIPT_DIR/references/chinook/Chinook.sql
